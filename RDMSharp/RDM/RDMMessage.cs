...
public class RDMMessage : IEquatable<RDMMessage>
{
    ...
    public RDMMessage(ERDM_NackReason nackReason)
    {
        _nackReason = nackReason;
        PortID_or_Responsetype = (byte)ERDM_ResponseType.NACK_REASON;
    }
    public RDMMessage(AcknowledgeTimer acknowledgeTimer)
    {
        _acknowledgeTimer = acknowledgeTimer;
        PortID_or_Responsetype = (byte)ERDM_ResponseType.ACK_TIMER;
    }
    public RDMMessage(AcknowledgeTimerHighRes acknowledgeTimerHighRes)
    {
        _acknowledgeTimer = acknowledgeTimerHighRes;
        PortID_or_Responsetype = (byte)ERDM_ResponseType.ACK_TIMER_HI_RES;
    }
    ...
    private ERDM_NackReason? _nackReason = null;
    ...
    public ERDM_NackReason? NackReason
    {
        get
        {
            ...
            _nackReason = Tools.DataToEnum<ERDM_NackReason>(ref data);
            return _nackReason;
        }
    }
    private IAcknowledgeTimer? _acknowledgeTimer = null;

    public IAcknowledgeTimer? AcknowledgeTimer
    {
        get
        {
            if (ResponseType != ERDM_ResponseType.ACK_TIMER && ResponseType != ERDM_ResponseType.ACK_TIMER_HI_RES)
                return null;

            if (_acknowledgeTimer != null)
    ...
    public byte PDL
    {
        get
        {
            switch (ResponseType)
            {
                case ERDM_ResponseType.NACK_REASON:
                case ERDM_ResponseType.ACK_TIMER:
                case ERDM_ResponseType.ACK_TIMER_HI_RES:
                    return 2;
            }
    ...
    public byte[] ParameterData
    {
        get
        {
            ...
            {
                case ERDM_ResponseType.NACK_REASON:
                    return Tools.ValueToData(_nackReason);
                case ERDM_ResponseType.ACK_TIMER:
                case ERDM_ResponseType.ACK_TIMER_HI_RES:
                    return Tools.ValueToData(_acknowledgeTimer);
            }
