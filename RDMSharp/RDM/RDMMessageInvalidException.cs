...
public class RDMMessageInvalidException : Exception
{
    ...
    internal static void ThrowIfInvalid(RDMMessage msg, ERDM_Command expectedCommand, params ERDM_Parameter[] expectedParameters)
    {
        ...
        if (expectedCommand.HasFlag(ERDM_Command.RESPONSE) && !msg.IsAck)
        {
            byte[] data = msg.ParameterData.ToArray();
            throw new RDMMessageInvalidException(msg, $"NACK Reason: {Tools.DataToEnum<ERDM_NackReason>(ref data)}");
        }
