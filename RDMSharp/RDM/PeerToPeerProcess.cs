...
public class PeerToPeerProcess : INotifyPropertyChanged
{
    ...
    private DataTreeBranch _responsePayloadObject = DataTreeBranch.Unset;
    public DataTreeBranch ResponsePayloadObject
    {
        get
        {
            return _responsePayloadObject;
        }
        private set
        {
            if (_responsePayloadObject == value)
                return;
            _responsePayloadObject = value;
            PropertyChanged?.Invoke(this, new PropertyChangedEventArgs(nameof(ResponsePayloadObject)));
        }
    }

    public MetadataJSONObjectDefine Define { get; private set; }
    private EPeerToPeerProcessState _state = EPeerToPeerProcessState.Waiting;
    public EPeerToPeerProcessState State
    {
        get
        {
            return _state;
    ...
    private ERDM_NackReason? _nackReason;
    public ERDM_NackReason? NackReason
    {
        ...
        private set
        {
            if (_nackReason == value)
                return;
            _nackReason = value;
            PropertyChanged?.Invoke(this, new PropertyChangedEventArgs(nameof(NackReason)));
        }
    }
    ...
    private ConcurrentBag<RequestResponseHistoryEntry> requestResponseHistory = new ConcurrentBag<RequestResponseHistoryEntry>();
    public IEnumerable<RequestResponseHistoryEntry> RequestResponseHistory => requestResponseHistory.ToList();

    internal event EventHandler<RequestResponseHistoryEntry> OnRequestResponseHistoryAdded;
    ...
    public async Task Run(AsyncRDMRequestHelper asyncRDMRequestHelper = null)
    {
        ...
        try
        {
            ...
            do
            {
                ...
                var historyEntry = new RequestResponseHistoryEntry(request);
                requestResponseHistory.Add(historyEntry);
                ...
                {
                    historyEntry.SetResponse(responseResult.Response, "Timeout");
                    State = EPeerToPeerProcessState.Failed;
                    return;
                }
                if (responseResult.Disposing)
                {
                    historyEntry.SetResponse(responseResult.Response, "Disposing");
                    State = EPeerToPeerProcessState.Failed;
                    return;
                }
                ...
                {
                    historyEntry.SetResponse(responseResult.Response, "NACK");
                    State = EPeerToPeerProcessState.Failed;
        ...
        {
            var last = requestResponseHistory.LastOrDefault();
            if (last is not null)
                if (last.State is null)
                    last.SetResponse(null, "Fail");
