...
public class TestPeerToPeerProcess
{
    ...
    public async Task Test_Get_LAMP_STRIKES()
    {
        ...
        async Task sendMessage(RDMMessage message)
        {
            ...
            RDMMessage response = count == 0 ?
                new RDMMessage(new AcknowledgeTimer(TimeSpan.FromSeconds(3)))
                {
                    Command = message.Command | ERDM_Command.RESPONSE,
                    DestUID = message.SourceUID,
                    SourceUID = message.DestUID,
            ...
            new RDMMessage()
            {
                ...
                SubDevice = message.SubDevice,
                TransactionCounter = message.TransactionCounter,
                ParameterData = MetadataFactory.GetResponseMessageData(parameterBag, new DataTreeBranch(new DataTree[] { new DataTree("strikes", 0, LAMP_STRIKES) })).First(),
