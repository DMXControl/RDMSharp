...
namespace RDMSharpTests.RDM.PayloadObject.SGM
{
    public class RefreshRateTest
    {
        ...
        {
            RefreshRate refreshRateRaw = new RefreshRate(1);
            ...
            Assert.That(refreshRateRaw.Equals(new RefreshRate(1)), Is.False);
            Assert.That(new RefreshRate(RefreshRate.FREQUENCY_MULTIPLYER).Equals(new RefreshRate(RefreshRate.FREQUENCY_MULTIPLYER - 1)), Is.False);
        }
    }
}